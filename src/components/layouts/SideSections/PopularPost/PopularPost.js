...
import popularBlogs from "./popularData";
    ...
    return (
        <>
            ...
            {popularBlogs.map((value) => (
                    <li className="w3-padding-16 popular-box" key={value.id} onClick={() => scrollToBlog(`blog${value.id}`)} style={{cursor:'pointer'}} >
                        <img src={value.image} alt={`Blog-${value.id}`} className="w3-left w3-margin-right popular-image" ></img>
